...
var App = (function () {
    ...
    App.prototype.subscribeToPubSub = function () {
        ...
        this.pubSub.subscribe(this.config.topic, sessionId, function (data) {
            if (!_this.isCorrectRepository(data)) {
                return;
            }
            if (!_this.isCorrectBranch(data)) {
                return;
            }
            _this.executeScript();
        });
    };
    App.prototype.isCorrectRepository = function (data) {
        return data.name === this.config.repository;
    };
    App.prototype.isCorrectBranch = function (data) {
        var _this = this;
        var refs = Object.keys(data.refUpdateEvent.refUpdates);
        return refs.find(function (r) { return r === _this.config.branch; }) ? true : false;
    };
    App.prototype.executeScript = function () {
        child_process.exec("sh " + this.config.shellScript, function (error, stdout, stderr) {
            if (error) {
